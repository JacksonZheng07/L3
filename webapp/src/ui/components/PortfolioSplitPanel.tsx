      ...
      <div className="px-5 pb-5">
        {!hasScores ? (
          ...
          <div className="space-y-3">
            {/* Column headers */}
            <div className="grid grid-cols-[1fr_auto_auto] gap-3 text-[9px] font-mono uppercase tracking-widest text-[#8b949e]/60 pb-1 border-b border-[#21262d]">
              <span>Mint</span>
              <span className="text-right w-20">Alloc</span>
            ...
            {eligibleScores.map((score) => {
              ...
              const barPct = score.allocationPct; // already capped to 40% by engine
              ...
              return (
                <div key={score.url} className="space-y-1.5">
                  ...
                  <div className="grid grid-cols-[1fr_auto_auto] gap-3 items-center">
                    ...
                    <span className="text-[10px] font-mono text-[#8b949e] text-right w-20">
                      {score.allocationPct.toFixed(1)}%
                    </span>
                    <span
                      className="text-xs font-mono font-semibold text-right w-28 tabular-nums"
                      style={{ color }}
                    >
            ...
            {/* Total row */}
            <div className="pt-3 mt-1 border-t border-[#21262d] grid grid-cols-[1fr_auto_auto] gap-3 items-center">
              <span className="text-[10px] font-mono text-[#8b949e]">
                Total ({eligibleScores.length} mints)
              </span>
              <span className="text-[10px] font-mono text-[#8b949e] text-right w-20">
              ...
              <span className="text-xs font-mono font-bold text-[#c9d1d9] text-right w-28 tabular-nums">
                {eligibleScores
                  .reduce((s, m) => s + Math.round((m.allocationPct / 100) * previewAmount), 0)
                  .toLocaleString()}
              </span>
            </div>
